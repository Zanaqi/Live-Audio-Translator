...
export async function GET(req: NextRequest, { params }: { params: { roomId: string } }) {
  try {
    ...
    const isTourist = user.role === 'tourist' && room.participants.some(p => p.userId === user.id);
...
export async function DELETE(req: NextRequest, { params }: { params: { roomId: string } }) {
  try {
    ...
    if (user.role !== 'guide') {
      return NextResponse.json({ error: 'Only guides can delete rooms' }, { status: 403 });
    }
    ...
    if (room.createdBy !== user.id) {
      return NextResponse.json({ error: 'You can only delete your own rooms' }, { status: 403 });
    }
