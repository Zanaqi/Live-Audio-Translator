...
import { Mic, MicOff, Users, Share2, Crown, RefreshCw, Trash2 } from 'lucide-react'
...
export default function GuideRoomPage() {
  ...
  useEffect(() => {
    const fetchRoomData = async () => {
      ...
      try {
        ...
        await connectToWebSocket(roomData.id, roomData.code, user.id, user.name);
        
      } catch (error) {
        console.error('Error fetching room data:', error);
        setError(error instanceof Error ? error.message : 'Failed to load room data');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const connectToWebSocket = async (roomId: string, roomCode: string, userId: string, userName: string) => {
    try {
