...
export interface ContextState {
  domain: ContextConfidence;
  topics: Map<string, ContextConfidence>;
  conversationHistory: string[];
  keyReferences: Map<string, ContextConfidence>;
}

export class ContextManager {
  private state: ContextState;
  private readonly MAX_HISTORY = 10; // Keep last 10 exchanges
  private readonly CONFIDENCE_DECAY = 0.95; // 5% confidence decay per minute
  private readonly DECAY_INTERVAL = 60000; // 1 minute
  private readonly MIN_CONFIDENCE = 0.3; // Threshold for context to be considered
  ...
  private updateDomainContext(text: string) {
  ...
  private updateTopicContext(text: string) {
  ...
  private extractKeyReferences(text: string) {
  ...
  public getTopTopics(limit: number = 3): {topic: string, confidence: number}[] {
